...
def lease_spider(max_pages):
    ...
    while page <= max_pages:
        url = 'http://chengde.58.com/cangkucf/pn' + str(page)
...
def get_single_item_data(item_url):
    ...
    f = open('58lease_warehouse_info_chengde.txt','a')
    ...
    for item_info in soup.findAll('ul', {'class':'info'}):
        ...
        strings = u"河北" +',' + u"承德" + ','+ item_name +',' +item_general_location + ',' + item_specific_location +',' + item_area + ',' + item_lease_fee
        s = re.sub('\s','',strings)
    ...
    h,r,w ={}, file('58lease_warehouse_info_chengde.txt'), file('58lease_warehouse_info_after_removral_chengde.txt','w')
